...
public class BoardDAO {
	...
	public boolean insertArticle(BoardDTO boardDTO) {
		...
		try {
			...
			// board_seq.nextval 발행을 처음하면 1부터 생성, re_ref는 bno와 동일한 값으로 넣으려고하니 board_seq.currval
	...
	public List<BoardDTO> getRows(PageDTO pageDTO){
		...
		try {
			...
			String sql = null;
			if(pageDTO.getKeyword().isEmpty() && pageDTO.getCriteria().isEmpty()) { // 키워드와 criteria에 값이 없다면
				// 전체
//				sql = "select bno,title,name,regdate,cnt,re_lev from board ";
//				sql += "order by re_ref desc, re_seq asc"; // 최신글 순 
				...
				// 검색
				//sql = "select bno,title,name,regdate,cnt,re_lev from board ";
				//sql += "where " +pageDTO.getCriteria()+ " like ? order by re_ref desc, re_seq asc"; //필드명에서는 ? 처리 못함 -> 변수로 사용
	...
	public int totalRows(PageDTO pageDTO) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if(rs.next()) {
				total = rs.getInt(1); //1번 칼럼에 있는거 가져와
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
	...
	// 답변
	// sql 구문 두 번 실행 DML 구문 : 반드시 commit을 해야 최종반영
	public boolean reply(BoardDTO updateDto) {
		boolean flag = false;
		try {
			con = getConnection();
			
			//"update board set re_seq = re_seq + 1 where re_ref=부모re_ref and re_seq > 부모re_seq;";
			...
			pstmt.executeUpdate(); // result 값을 의미 없어 걍 실행
			...
			rollback(con); // insert 실패하면 update도 안되로록 하나의 트랜잭션으로 묶음 그래서 한번에 커밋함
			e.printStackTrace();
		}finally {
			close(con, pstmt);
		}
	...
	public List<BoardDTO> searchRows(String criteria, String keyword) {
		...
		try {
			...
			sql += "where " +criteria+ " like ? order by re_ref desc, re_seq asc"; //필드명에서는 ? 처리 못함 -> 변수로 사용
			pstmt = con.prepareStatement(sql);
