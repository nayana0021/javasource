...
<%
	// 세션에 로그인정보가 담겨있는지 확인
	LoginDTO loginDto = (LoginDTO)session.getAttribute("loginDto"); // 오브젝트로 담기기 때문에 형변환해야 함
	//out.print("<h3>"+loginDto+"</h3>");
	if(loginDto != null){
%>
<%-- 로그인 정보가 존재한다면 보여질 영역 --%> <!-- 어떤 페이지를 가도 세션이 가지고 있는 로그인정보를 유지 -->
...
%>
<!-- 로그인을 하면 -> 세션에 담는다 : 브라우저 닫기 전까지 유지  -->
